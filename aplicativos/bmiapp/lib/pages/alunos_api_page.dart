...
import 'package:bmiapp/service/api.dart';
import 'package:flutter/material.dart';

class AlunosAPIPage extends StatefulWidget {
  const AlunosAPIPage({super.key});

  @override
  State<AlunosAPIPage> createState() => _AlunosAPIPageState();
}

class _AlunosAPIPageState extends State<AlunosAPIPage> {
  ...
  Future<void> _saveNota() async {
    var result = await apiClient.getNotas();
    print(result);
    ...
    if (_formKey.currentState!.validate()) {
      if (!isEditing) {
        ...
        await apiClient.updateNota(_nota);
      }
  ...
  Future<void> _removeNota(Nota nota) async {
    // dbHelper.deleteNota(nota);
    await apiClient.deleteNota(nota.id!);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('App Alunos - API Version'),
      ),
    );
  }
}
